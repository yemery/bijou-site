  ...
  return (
    <div className="w-full h-[80px] bg-white border-b">
      <div className="md:max-w-[1480px]  max-w-[600px]  m-auto w-full h-full flex justify-between items-center  px-4">
        <div className="font-sans font-semibold">JewelJazz</div>
        {/* lg */}
        <div className="hidden md:flex items-center ">
          <ul className="flex justify-between items-center  bg-transparent  px-6 gap-9">
            {navbarItems.map((e) => (
              <>
                <li key={e.id} className="flex flex-col items-center hover:underline">
                  {" "}
          ...
          <ul className="flex flex-col gap-7 justify-items-start">
            {navbarItems.map((e) => (
              <>
                <li key={e.id} >
                  {" "}
                  <Link className="text-xl flex items-end  " to={e.to}>{e.label} <span><MdArrowOutward  size={13}/></span></Link>
